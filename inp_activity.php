...
<tr>
<td><font color=white size=3><b>Add Activity</b></font></td>
</tr>
</table><p>
...
$actor_type_id=$_POST["actor_type_id"];

$hostn=$_POST["hostname"];
$system_status_type_id=$_POST["system_status_type_id"];
$ipaddr=$_POST["ip_addr"];
$OpCo_id=$_POST["OpCo_id"];
...
$act_ins='insert into activity(activity_date,activity_type_id,actor_type_id,description,created_by) values("'.
         $activity_date.'",'.$activity_type_id.','.$actor_type_id.',"'.$description.'","Test")';

if ($conn->query($act_ins) === FALSE) {
   echo '<b><font color=red>Error entering activity: ' . $act_ins . '</font></b><br>' . $conn->error;
   include 'http://10.1.3.94/ci/ifrm_activity.php';
   }
...
$chk_host = $conn->query('select id from systems where hostname = "'.$hostn.'"');
